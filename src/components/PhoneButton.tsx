...
const PhoneButton: React.FC<PhoneButtonProps> = ({
  ...
  const numberToDisplay = displayNumber || formatPhoneNumber(phoneNumber);
  const telLink = `tel:${phoneNumber.replace(/\s/g, '')}`;

  return (
    <a
      href={telLink}
      className={`inline-flex items-center py-3 px-6 rounded-full border text-lg font-semibold transition-all duration-300 cursor-pointer hover:active:translate-y-0 hover:active:shadow-md ${
        inverted
          ? 'text-primary border-primary hover:bg-primary hover:text-secondary'
          : 'text-secondary border-white hover:bg-white hover:text-primary'
      }`}
    ...
    >
      ...
      <span className="leading-none">{numberToDisplay}</span>
    </a>
  );
};
